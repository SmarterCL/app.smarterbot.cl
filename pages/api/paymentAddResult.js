...
export default async function handler(request, response) {
  ...
  if (!process.env.MAILGUN_API_KEY || !process.env.FLOW_API_KEY || !process.env.FLOW_SECRET_KEY) {
  ...
  try {
    ...
    if (accError) {
      console.error('Database error in paymentAddResult:', accError);
      return response.redirect(`/pay/${request.query.uid}/add/error`);
    }
    ...
    if (flowResponse.status == 2) {
      await db
        ...
        .update({
          ...
          paymentStatus: "DONE",
          expirationDate: moment().add(1, "years").toISOString(), // Changed toDate() to toISOString() for consistency
        })
      ...
      try {
        await mg.messages.create("rut.smarterbot.store", {
        ...
        return response.redirect(`/pay/${request.query.uid}/add/success`);
      } catch (emailError) {
        console.error('Email sending error in paymentAddResult:', emailError);
        return response.redirect(`/pay/${request.query.uid}/add/success`);
      }
    } else {
      return response.redirect(`/pay/${request.query.uid}/add/error`);
    ...
    console.error('Payment add result error:', error);
    return response.redirect('/pay/add/error');
  }
}
