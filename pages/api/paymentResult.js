...
export default async function handler(request, response) {
  ...
  try {
    ...
    const { data: documentSnapshot, error } = await db
      .from("accounts")
      .select("*")
    ...
    if (error) {
      console.error('Database error in paymentResult:', error);
      return response.redirect(`/pay/${request.query.uid}/error`);
    }
    ...
    if (flowResponse.status == 2) {
      ...
      try {
        ...
        console.error('Email sending error in paymentResult:', emailError);
        return response.redirect(`/pay/${request.query.uid}/success`);
      }
    } else {
      ...
      try {
        ...
        console.error('Email sending error for failed payment in paymentResult:', emailError);
      }
