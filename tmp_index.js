                         ...
                         <div className="relative bg-white rounded-[50px] shadow-[0_40px_100px_-20px_rgba(0,0,0,0.15)] border border-white overflow-hidden p-6 sm:p-10">
                             ...
                             <div className="flex p-1 bg-slate-100 rounded-full mb-8">
                                 <button 
                                     ...
                                     className={`flex-1 py-3 px-4 rounded-full text-sm font-bold transition-all ${activePlanTab === 'demo' ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}
                                 >
                                     Demo
                                 </button>
                                 <button 
                                     onClick={() => setActivePlanTab('enterprise')}
                                     className={`flex-1 py-3 px-4 rounded-full text-sm font-bold transition-all ${activePlanTab === 'enterprise' ? 'bg-[#FFCE00] text-slate-900 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}
                                 >
                                     Enterprise
                                 </button>
                             </div>
                             ...
                             {activePlanTab === 'demo' ? (
                                <div className="space-y-6 animate-in fade-in zoom-in duration-300">
                                    ...
                                    <a href="/login" className="w-full h-14 bg-slate-900 hover:bg-slate-800 text-[#FFCE00] font-black rounded-full flex items-center justify-center transition-all mt-6 shadow-xl hover:scale-[1.02]">
                                        Inscríbete gratis
                                    </a>
                                </div>
                             ) : (
                                <div className="space-y-6 animate-in fade-in zoom-in duration-300">
                                    ...
                                    <a href="/subscribe/pro" className="w-full h-14 bg-[#FFCE00] hover:bg-[#E0B800] text-slate-900 font-black rounded-full flex items-center justify-center transition-all mt-6 shadow-xl hover:scale-[1.02]">
                                        Comprar suscripción
                                    </a>
                                </div>
                             )}
               ...
               <div className="flex flex-col lg:flex-row items-center gap-12">
                   ...
                   <div className="w-full lg:w-1/2 text-center lg:text-left">
                       ...
                       <div className="flex flex-col sm:flex-row items-center justify-center lg:justify-start gap-4">
                           <a href="/preguntas-frecuentes" className="h-14 px-8 bg-slate-900 text-white font-black rounded-full flex items-center justify-center hover:bg-slate-800 transition-colors">
                               ¿Cómo funciona?
                           </a>
                       </div>
                   </div>
                       ...
                       <div className="max-w-[500px] mx-auto">
                           <div className="flex justify-center mb-6">
                               <div className="p-1 items-center bg-slate-100 rounded-full inline-flex">
                                   <button
                                       className={`px-6 py-2.5 rounded-full text-sm font-bold transition-all flex items-center justify-center ${activeMedia === 'video' ? 'bg-[#FFCE00] text-slate-900 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}
                                       onClick={() => setActiveMedia('video')}
                                   >
                                       Video Explicativo
                                   </button>
                                   <button
                                       className={`px-6 py-2.5 rounded-full text-sm font-bold transition-all flex items-center justify-center ${activeMedia === 'image' ? 'bg-[#FFCE00] text-slate-900 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}
       ...
       {showVideoModal && (
           ...
           >
               <div className="relative w-full max-w-4xl max-h-[90vh] rounded-3xl overflow-hidden shadow-2xl bg-black" onClick={e => e.stopPropagation()}>
                    <button
                        className="absolute z-10 top-4 right-4 w-10 h-10 bg-white/10 hover:bg-white/20 backdrop-blur-md rounded-full flex items-center justify-center text-white transition-colors"
                        onClick={() => setShowVideoModal(false)}
                    >
                        <X className="w-5 h-5" />
                    </button>
